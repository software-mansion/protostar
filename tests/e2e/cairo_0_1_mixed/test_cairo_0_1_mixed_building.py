...
def test_cairo1_build_with_contract_names_separate_builds(
    ...
    for contract in contracts:
        ...
        assert set(built_files) == {
            ...
            Path("build") / (contract + ".class_hash"),
            Path("build") / (contract + ".compiled_class_hash"),
        }
...
def test_cairo1_build_with_contract_names_build_together(
    ...
    for contract in contracts:
        ...
        expected_built_files.add(Path("build") / (contract + ".class_hash"))
        expected_built_files.add(Path("build") / (contract + ".compiled_class_hash"))
...
def test_cairo0_build_with_contract_names_separate_builds(
    ...
    for contract in contracts:
        protostar(["build-cairo0", "--contract-name", contract])
        build_path = Path("build")
        assert build_path.exists()
...
def test_cairo0_cairo1_build_mixed(
    ...
    for contract in cairo0_contracts:
        output = protostar(["build-cairo0", "--contract-name", contract])

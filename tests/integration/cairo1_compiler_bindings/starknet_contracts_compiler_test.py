...
def test_contract_to_casm(datadir: Path):
    result = cairo1.compile_starknet_contract(
        datadir / "basic_starknet_contract.cairo",
    )
    assert result
    result = cairo1.compile_starknet_contract(datadir / "basic_starknet_test.cairo")
    assert result


def test_cairo_path_for_starknet_contract(datadir: Path, shared_datadir: Path):
    casm_contents = cairo1.compile_starknet_contract(
    ...
    with pytest.raises(Exception):
        cairo1.compile_starknet_contract(
            input_path=datadir / "starknet_project" / "basic_starknet_contract.cairo",
        )

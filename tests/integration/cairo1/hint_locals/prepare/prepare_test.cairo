...
#[test]
fn test_prepare_doesnt_move_calldata() {
    let class_hash = declare('minimal_with_args').unwrap();

    let mut calldata = ArrayTrait::new();
    calldata.append('name');
    let prepare_result = prepare(class_hash, @calldata).unwrap();

    // This should work if calldata is not moved to prepare
    assert(calldata.len() == 1_u32, 'constructor_calldata size == 1');
}

from typing import TYPE_CHECKING

from .call_structs import CallInput, CallOutput

if TYPE_CHECKING:
    from protostar.starknet_gateway import GatewayFacade, DataTransformerPolicy


class CallUseCase:
    def __init__(
        self,
        gateway_facade: "GatewayFacade",
        data_transformer_policy: "DataTransformerPolicy",
    ) -> None:
        self._gateway_facade = gateway_facade
        self._data_transformer_policy = data_transformer_policy

    async def execute(self, input_data: CallInput) -> CallOutput:
        cairo_calldata = (
            await self._data_transformer_policy.transform_entrypoint_input_to_cairo(
                address=input_data.address,
                selector=input_data.selector,
                calldata=input_data.inputs,
                contract_abi_service=input_data.contract_abi_service,
            )
        )
        response_cairo_data = await self._gateway_facade.send_call(
            address=input_data.address,
            selector=input_data.selector,
            cairo_calldata=cairo_calldata,
        )
        response_human_data = await self._data_transformer_policy.transform_entrypoint_output_to_human_if_abi_found(
            data=response_cairo_data,
            address=input_data.address,
            selector=input_data.selector,
            contract_abi_service=input_data.contract_abi_service,
        )
        return CallOutput(
            cairo_data=response_cairo_data,
            human_data=response_human_data,
        )

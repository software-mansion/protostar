...
class CairoStruct:
    ...
    def __getattr__(self, name: str) -> "CairoValueType":
        ...
        raise KeyError(f"'{name}' is not a member of this CairoStruct.")

    def __setattr__(self, name: str, value: Any) -> None:
        if name != "_ordered_dict":
            raise ValueError("CairoStruct is immutable.")
        super().__setattr__(name, value)

    def _set(self, name: str, value: "CairoValueType") -> None:
        if not isinstance(cast(Any, value), VALID_CAIRO_TYPES):
            raise TypeError(f"{type(value).__name__} is not a valid CairoType.")
    ...
    def __str__(self) -> str:
        stack: list[tuple[Union[CairoValueType, str], int]] = [(self, 0)]
        result: list[str] = []

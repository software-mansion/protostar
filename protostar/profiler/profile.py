...
class ProfilerContext:
    ...
    def build_step_samples(self, instructions: List[Instruction], tracer_data: TracerData):
        step_samples = []
    ...
    def not_accessed(self, accessed_addresses, segments: MemorySegmentManager, segment_offsets) -> List[int]:
    ...
    def build_memhole_samples(self, instructions: List[Instruction], tracer_data: TracerData, accessed_memory, segments: MemorySegmentManager, segment_offsets):
        accessed_by = {}
        pc_to_callstack = {}
        ...
        for address in not_acc:
            pc = blame_pc(address)
            callstack = [self.find_instruction(instructions, frame_pc) for frame_pc in pc_to_callstack[pc]]
            samples.append(Sample(value=1, callstack=callstack))
        return samples
    
def build_profile(tracer_data: TracerData, segments: MemorySegmentManager, segment_offsets, accessed_memory) -> RuntimeProfile:
    ...
    memhole_samples = builder.build_memhole_samples(instructions_list, tracer_data, accessed_memory, segments, segment_offsets)
...
def profile_from_tracer_data(tracer_data: TracerData, runner: CairoFunctionRunner):
    profile = build_profile(tracer_data, runner.segments, runner.segment_offsets, runner.accessed_addresses)

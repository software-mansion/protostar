...
class MigratorContractIdentifierResolver:
    ...
    def resolve(self, contract_identifier: str, build_output_dir: Path) -> Path:
        if "." in contract_identifier:
            contract_path = Path(contract_identifier)
            return checked_contract_path(contract_path)

        return self._get_built_contract_by_name(contract_identifier, build_output_dir)

    def _get_built_contract_by_name(
        self, contract_name: str, build_output_dir: Path
    ) -> Path:

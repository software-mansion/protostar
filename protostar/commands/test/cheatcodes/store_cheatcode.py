...
class StoreCheatcode(Cheatcode):
    ...
    def normalize_address(self, addr):
        return addr if addr < ADDR_BOUND else addr
    ...
    def store(self, target_contract_address: int, var: str, value: int, key: Optional[CairoArguments]):
        key = key or []
        if isinstance(constructor_calldata, collections.Mapping):
            constructor_calldata = self.transform_data_to_cairo_format(
                target_contract_address, key
            )

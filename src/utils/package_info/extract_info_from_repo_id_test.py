...
from src.utils.package_info.extract_info_from_repo_id import (
    _map_ssh_to_url,
...
def test_incorrect_url():
    with pytest.raises(package_info_extractor_exceptions.IncorrectURL):
...
def test_failure_at_unknown_repo_id_format():
    with pytest.raises(package_info_extractor_exceptions.InvalidPackageName):

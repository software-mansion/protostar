...
fn test_increase_balance() {
    ...
    assert(result_before[0] = 0, 'Invalid balance');

    let mut invoke_calldata = ArrayTrait::new();
    invoke_calldata.append(42).unwrap();
    ...
    assert(result_after[0] = 42, 'Invalid balance');
}
...
fn test_cannot_increase_balance_with_negative_value() {
    ...
    let result_before = call(contract_address, 'get_balance', ArrayTrait::new()).unwrap();
    assert(result_before = 0, 'Invalid balance');

    let mut invoke_calldata = ArrayTrait::new();
    invoke_calldata.append(-10000).unwrap();
